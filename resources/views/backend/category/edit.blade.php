                ...
                <form action="{{ route('admin.category.update') }}" method="POST" class="">
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control @error('name')
                            is-invalid
                        @enderror" type="text" id="name" name="name" value="{{ $categoryDetail->name }}">
                        @error('category')
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control @error('slug')
                            is-invalid
                        @enderror" type="text" id="slug" name="slug" value="{{ $categoryDetail->slug }}" readonly>
                        @error('slug')
                        <div class="invalid-feedback">
                            {{ $message }}
                        </div>
                    ...
                    <div class="form-group">
                        ...
                        @enderror" id="parent" name="parent">
                            <option value="" {{ ($categoryDetail->parent == null) ? 'selected': ''}}>Null (Doesn't Have Parent)</option>
                            @foreach ($categories as $cat)
                            <option value="{{ $cat->id }}" {{ ($categoryDetail->parent == $cat->id) ? 'selected': ''}}>{{ $cat->name }}</option>
                            @endforeach
